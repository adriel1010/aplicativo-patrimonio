...
public class FormularioPatrimonio extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (extras != null) {
            idLocaisInventario = extras.getLong("idLocaisInventario");
        }
    ...
    public void cadastrarPatrimonio(View view){
        ...
        LocalInventario localConferir = new LocalInventario();
        localConferir = LocalInventario.findById(LocalInventario.class, idLocaisInventario);
          ...
          if(localConferir != null) {
              ...
              if (objCodPat.getText().toString().length() > 1) {
                  ...
                  if (EquipamentoInventario.findWithQuery(EquipamentoInventario.class, "Select * from EQUIPAMENTO_INVENTARIO e inner join tombamento t on e.tombamento = t.id  inner join LocaL_Inventario l on e.local_Inventario = l.id  where t.codigo = ? and l.id = ? ", objCodPat.getText().toString().trim(), String.valueOf(localConferir.getId())).size() > 0) {
